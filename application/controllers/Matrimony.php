 ...
 // For new image on click refresh button.
  ...
  //otp generation
   ...
   public function photoAdd(){
       ...
       if($save){
                $config['upload_path'] = 'uploads/'.$profilecode; 
                ...
                $this->load->library('upload', $config);
                if (!is_dir('uploads/'.$profilecode)) {
                    mkdir('uploads/' . $profilecode, 0777, TRUE);
                    } 
                $this->upload->initialize($config);
                if(!$this->upload->do_upload('image')){
                    ...
                    $config1 = array(
                            ...
                            'new_image' => 'uploads/'.$profilecode.'/thumbnail', //save as new image //need to create thumbs first
                            'maintain_ratio' => false,
                            'width' =>150,
                            'height' =>150,
                            'thumb_marker'=>true,
                    );
                if (!is_dir('uploads/'.$profilecode.'/thumbnail')) {
                     mkdir('uploads/' . $profilecode.'/thumbnail', 0777, TRUE);
                    }  
               $this->load->library('image_lib', $config1); //load library
               $this->image_lib->resize(); //generating thumb
               $img =array(
               'orgimage' =>'uploads/'.$profilecode.'/'.$fimage,
               'thumbimage' => 'uploads/'.$profilecode.'/thumbnail/'.$fimage,
               );
